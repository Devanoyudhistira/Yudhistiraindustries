<x-layout>
    ...
    <main class="mt-8 flex flex-col" x-data="{blogform:false}" >
        <form  method="GET" class="mt-13 self-center relative flex gap-4">
            ...
            @endif
            <input class="border-3 h-13 py-2 pl-3 mr-3 w-90 rounded-2xl border-zinc-950" name="search" type="search" placeholder="search">
        ...
        <div class="mt-8 grid w-screen grid-cols-3 gap-x-4 px-3 py-2 place-items-center relative place-content-center">
            @if (count($datablogs) <= 0)
            <div class="top-[30%] left-[33%] absolute" >
            <h1 class="font-zalando text-7xl tracking-wide font-semibold "> news not found </h1>
            <p class="font-zalando text-3xl tracking-wide font-thin " > try another keyword </p>
            </div>
            @endif
