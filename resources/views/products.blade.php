<x-layout>
    ...
    <main class="h-auto w-screen">
        ...
        <article x-data="{ open: false,productname:'' }" class="grid grid-cols-4 gap-2 pl-9">
            @foreach ($products as $product)
                <div id="productCard" x-data="{ productdata:'{{ $product['productname'] }}' }" class="w-54 h-75 flex flex-col items-center border-2 border-zinc-100 bg-zinc-900">
                    ...
                    <button x-on:click="productname = productdata;open = true"
            ...
            <div x-transition x-show="open"
                ...
                <button class="text-[80px] -mt-15 self-start items-self-start justify-self-start" x-on:click="open = false"> <i class="bi bi-x font-black text-red-600"></i> </button>
                <div class="flex w-full h-full -mt-10" >
                    <img src="{{ asset("image/product3.jpg") }}" id="productimage" class="w-[70%] h-[70%] object-cover"></img>
                    <div class="flex-col flex w-full h-[70%] bg-sky-100">
                      <h1 class="text-4xl font-bebas font-medium " x-text="productname">  </h1>
                ...
                <button class="w-[60%] ml-60 justify-self-center items-self-center rounded-xl border-2 hover:border-black hover:bg-zinc-200 hover:text-zinc-900 duration-100 -mt-40 text-center bg-zinc-900 text-zinc-100 tracking-wider font-bebas font-semibold text-xinc-200 align-center text-2xl"> <i class="bi bi-cart"></i> buy </button>
            </div>
        </article>
    </main>
    <script>
        function buytab(productname,targetname){        
